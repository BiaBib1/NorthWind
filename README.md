...
2. **Entra en la consola de PostgreSQL**
   ```bash
   psql -U postgres      # Modifica el nombre del usuario si es necesario
   ```
...
5. **Importa el dump**
   ```bash   
   psql -U postgres -d northwind_mod -f northwind_dump.sql     # Modifica el nombre del usuario si es necesario
   ```
